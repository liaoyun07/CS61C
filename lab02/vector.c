...
vector_t *vector_new() {
    ...
    // not retval (ptr size)
    retval->data = (int*) calloc(retval->size,sizeof(int));/* YOUR CODE HERE */;
...
void vector_delete(vector_t *v) {
    /* YOUR SOLUTION HERE */
    //free can't free mem recursioning
    //空指针不能解引用
    if(v == NULL)
        return;
    free(v->data);
...
void vector_set(vector_t *v, size_t loc, int value) {
    ...
    if(loc < v->size){
        ...
        int* newMem = NULL;
        newMem = malloc(2*loc*sizeof(int));
        if(newMem == NULL){
            free(v); //退出之前把分配过的内存带上
            allocation_failed();
        }
